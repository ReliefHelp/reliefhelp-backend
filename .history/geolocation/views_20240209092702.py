...
def get_current_user_weather(request):
    ...
    api_key = "bff4631f7dbbfbbf9c127fbc2d4e3edf"
...
@api_view(["GET"])
def get_current_user_climate(request):
    """
    API endpoint to retrieve current weather data for the user's location
    Parameters
    None
    ...
    def get_climate_data(api_key, latitude, longitude):
        """
        Helper function to fetch weather data from the OpenWeatherMap API.
        Parameters
        api_key (str): OpenWeatherMap API key.
    ...
    # Get current weather data
    weather_data = get_climate_data(api_key, latitude, longitude)

    if weather_data:
        return Response({"climate_data": weather_data})
    else:
        return Response({"detail": "Weather data not found"}, status=404)
